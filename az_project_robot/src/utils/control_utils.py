...
def set_motors_direction(robot, command, vx, vy, theta):
    if command == 'go_forward':
        ...
        robot.vxg, robot.vyg, robot.theta_d, robot.turn = 0, 0, -theta, 1
    elif command == 'rotate_right':
        robot.vxg, robot.vyg, robot.theta_d, robot.turn = 0, 0, theta, 1
    control(robot)

def control(robot):
    robot.backward_kinematics() 

    # Cập nhật tốc độ cho từng động cơ
    Motors.set_speed("motor_1", max(0, min(abs(robot.v1), 1)))
    Motors.set_speed("motor_2", max(0, min(abs(robot.v2), 1)))
    Motors.set_speed("motor_3", max(0, min(abs(robot.v3), 1)))
    Motors.set_speed("motor_4", max(0, min(abs(robot.v4), 1)))

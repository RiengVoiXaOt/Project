...
def user_input_listener(modes):
    """Lắng nghe đầu vào từ người dùng để chuyển đổi chế độ."""
    while True:
        user_input = input("Nhấn 'p' để thoát khỏi chế độ tự động: ").strip()
        if user_input.lower() == 'p':
            modes.exit_automatic_mode()
            break
    ...
    try:
        while True:
            ...
            if user_input == '1':
                ...
                automatic_thread = Thread(target=modes.automatic_mode)  # Bắt đầu chế độ tự động trong luồng riêng
                automatic_thread.start()
                listener_thread = Thread(target=user_input_listener, args=(modes,))  # Luồng lắng nghe đầu vào
